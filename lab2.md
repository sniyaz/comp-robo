...
## 💻 Coding Question 2: NumPy and ROS Subscribers

In this question, we will compute the Euclidean norm in two ways: with regular Python for loops and with functions from NumPy. We will also create a ROS subscriber to listen to the car's pose.
...
*   **Q2.1:** Complete the `norm_python` function in `introduction/src/introduction/listener.py`. 
...
*   **Q2.2:** In the same file, complete the `norm_numpy` function. 
...
*   **Q2.3:** In the `PoseListener` class in `introduction/src/introduction/listener.py`, initialize a subscriber to the car’s pose topic. 
...
*   **Q2.4:** Fill in the `PoseListener.callback` method.
...
2.  **In another terminal**, launch the path publisher to have the car follow a "figure 8" plan:
    ...
    ```
3.  **Q2.5:** In `scripts/pose_listener`, use your `norm_numpy` function to compute the car’s distance to the origin for all the xy-locations captured by the `PoseListener`. 
...
*   **Q2.1 & Q2.2:** 10 points if `norms.py` passes.
*   **Q2.3 & Q2.4:** 5 points if `pose_listener.test` passes.
*   **Q2.5:** 5 points for the correct implementation and plots.
*   **Write-Up:** 5 points.
