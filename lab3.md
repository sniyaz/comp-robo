...
To explain these plots: we take the initial position of the robot, and apply your motion model (which is non-deterministic) a bunch of times to get a bunch of different result states. This is actual a key step of something called the *Particle Filter Algorithm* for HMMs, which we'll learn more about in later lectures (and which you probably got an introduction to in CS 5100). But, for now, just admire the pretty plots.

**Some food for throught**: Why does the motion model produce more particles within 10cm of the noise-free model prediction in Figure 2 than Figure 3?

**Deliverable:** Tune your motion model parameters to get as close to the staff parameters as you can. To explain your tuning process, please save three versions of the figure with `[3.0, 0.4, 0.5]` (the one with more noise) that were generated by different parameter sets. Call these `mm1.png`, `mm2.png`, `mm3.png`: the last should be your "best" parameter set.

In your writeup, explain how you tuned parameters to get from `mm1.png` to `mm3.png`. That said, your `mm3.png` doesn’t need to match ours exactly (just try to get close). We'll try to be generous in grading this.
